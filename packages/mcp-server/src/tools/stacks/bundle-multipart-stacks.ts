...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      manifest: {
        type: 'string',
        description: 'JSON string containing bundler manifest (must include entrypoint)',
      },
      tarball: {
        type: 'string',
        description: 'Tar.zst project archive',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Benchify, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.stacks.bundleMultipart(body)));
};

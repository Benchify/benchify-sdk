...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      tarball_filename: {
        type: 'string',
        description: 'Optional tarball filename',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Benchify, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.stacks.reset(id, body)));
  } catch (error) {
    if (isJqError(error)) {
